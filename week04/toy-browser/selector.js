...
function matchSelector(selector, element) {
    if (selector.type === 'tagName') {
        ...
        return !!element.classList.find(clz => clz === selector.value);
...
function matchAncestor(selectors) {
    if (!currentElement || currentElement === document) return false;
    if (matchSelectors(selectors, currentElement.parentNode)) return true;
    return matchAncestor(selectors, currentElement.parentNode);
}

function matchNeighbor(selectors) {
    if (!currentElement.previousSibling) return false;
    return matchSelectors(selectors, currentElement.previousSibling);
}

function matchBrother(selectors) {
    if (!currentElement.previousSibling) return false;
    if (matchSelectors(selectors, currentElement.previousSibling)) return true;
    return matchBrother(selectors, currentElement.previousSibling);
}

function matchParent(selectors) {
    if (!currentElement.parentNode) return false;
    return matchSelectors(selectors, currentElement.parentNode);
}

function match(selector, element) {
    parseSelector(selector);
    let bottom = current;
    let matched = matchSelectors(bottom.selectors, element);
    while (bottom && matched) {
        if (bottom.ancestor) {
            bottom = bottom.ancestor;
            matched = matchAncestor(bottom.selectors);
        } else if (bottom.parent) {
            bottom = bottom.parent;
            matched = matchParent(bottom.selectors);
        } else if (bottom._neighbor) {
            bottom = bottom._neighbor;
            matched = matchNeighbor(bottom.selectors);
        } else if (bottom._brother) {
            bottom = bottom._brother;
            matched = matchBrother(bottom.selectors);
        } else {
            bottom = null;
        }
    }
...
match('div .div2', document.querySelector('.div2'));
